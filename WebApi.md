...
На все методы написаны тесты для Postman. Чтобы ими воспользоваться, импортируй в Postman
коллекцию `/WebApi/Samples/UsersApi.postman_collection.json`.

После импорта коллекции, нужно задать переменную `baseUrl`. Она должна быть равна адресу,
на котором ты будешь запускать API. Например, `http://localhost:4442`.
Запусти приложение `WebApi` под отладкой, чтобы узнать этот адрес.
...
что это поле было задано на клиенте.
```cs
[Required]
...
Сделай так, чтобы `Login` был обязательным с помощью `Required`
и добавь проверку, что он состоит только из цифр и букв с помощью `AddModelError`.
...
В остальном тут все должно быть интуитивно понято. Тем более, что у тебя есть тесты.
...
Важно, что метод PUT работать по-разному.
...
Подсказка: Для вставки новой сущности в репозиторий используй метод `UpdateOrInsert`,
